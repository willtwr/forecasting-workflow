...
def plot_stacked_bar(
    ...
    """
    assert all(len(sublist) == len(data[0]) for sublist in data[1:]), (
    ...
    for i in range(n_groups):
        plt.bar(
            ...
            width,
            bottom=bottom,
            label=None if category_labels is None else category_labels[i],
        )
        bottom += data[i]
    ...
    plt.ylabel(ylabel)
    plt.xticks(
        ind, group_labels, rotation=rotate, ha="right" if rotate > 0 else "center"
    )
    plt.legend()
    plt.show()
